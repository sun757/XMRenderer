...
public:
    ...
    Material material;                  // colors material

    // constructor, expects a filepath to a 3D model.
    Model(string const& path,string const& file, bool gamma = false) : gammaCorrection(gamma)
    {
        loadModel(path,file);
    }
    ...
    void Draw(Shader& shader)
    {
        for (unsigned int i = 0; i < meshes.size(); i++)
            meshes[i].DrawWithTexture(shader);
    }
    void DrawWithColor(Shader& shader, bool withColor, glm::vec3 color)
    {
        for (unsigned int i = 0; i < meshes.size(); i++)
            meshes[i].DrawWithColor(shader, withColor, color);
    }
private:
    // loads a model with supported ASSIMP extensions from file and stores the resulting meshes in the meshes vector.
    void loadModel(string const& path, string const& file)
    {
        // read file via ASSIMP
        Assimp::Importer importer;
        string sourceFile = path + file;
        const aiScene* scene = importer.ReadFile(sourceFile, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
        // check for errors
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        Material   colors = loadMaterial(material);
        // 1. diffuse maps
    ...
    vector<Texture> loadTextures(aiMaterial* mat, aiTextureType type, string typeName)
    {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
                ...
                Texture2D texture(texture_path);
